...
(function() {
        ...
        if (anyStripped) {
          ...
          try {
            ...
            var names = files.filter(function(f, idx) { return processed[idx] !== f; })
                             .map(function(f) { return f.name; }).join(', ');
            if (names) showNotification('Cleaned ' + names + ' (standard uploads)');
          } catch(e) { console.debug('[MetaStrip] Could not replace files (read-only input):', e); }
        }
      }).catch(function(err) { console.debug('[MetaStrip] Strip error in drop handler:', err); });
    });
  }
  ...
  function syncEnabledState() {
    ...
    } catch(e) { console.debug('[MetaStrip] Could not dispatch toggle event:', e); }
  }
