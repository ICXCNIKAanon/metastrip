...
function stripContainer(buffer: ArrayBuffer): ArrayBuffer {
  if (!isMp4(Buffer.from(buffer))) throw new Error("Not a valid MP4/MOV file");
...
export function stripMp4(input: Buffer): StripMp4Result {
  ...
  const output = Buffer.from(stripContainer(buffer));
  return { output, categories: input.equals(output) ? [] : ["metadata"] };
}
