...
export interface StripWebpOptions {
  /** When false, removes ICCP color profile chunk. Defaults to true. */
  preserveIcc?: boolean;
}
...
export function isWebp(buffer: ArrayBuffer): boolean {
  ...
  return (
    view.getUint32(0, true) === CC_RIFF &&
    view.getUint32(8, true) === CC_WEBP
  );
}
...
export function stripWebp(
  ...
  while (offset < len) {
    ...
    if (chunkEnd > len) {
      ...
      // This is a lenient parse; real WebP parsers often do the same.
      throw new Error('Truncated WebP chunk: file was not modified');
    }

    const keep = shouldKeepChunk(cc, preserveIcc);
  ...
  for (const [start, end] of chunks) {
    ...
    if (
      ...
      const cc = new DataView(buffer).getUint32(start, true);
      if (cc === CC_VP8X) {
  ...
  if (vp8xOutputOffset !== -1 && vp8xOutputOffset < out.byteLength) {
    ...
    const updated = flagsByte & ~((1 << FLAG_EXIF_BIT) | (1 << FLAG_XMP_BIT) | (preserveIcc ? 0 : 0x20));
 ...
 */
function shouldKeepChunk(cc: number, preserveIcc: boolean): boolean {
  // Strip EXIF and XMP metadata chunks.
  if (cc === CC_EXIF) return false;
  if (cc === CC_XMP)  return false;

  // ICCP — kept unless caller opts out.
  if (cc === CC_ICCP) return preserveIcc;
